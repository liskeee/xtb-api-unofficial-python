...
## Task 1: Bump version and draft CHANGELOG

**Files:**
- Modify: `pyproject.toml`
- Modify: `CHANGELOG.md`
- Test: `tests/test_version.py` (existing — re-run, no new code)
...
(Only the `version` line changes; leave everything else alone.)

- [ ] **Step 2: Add CHANGELOG entry**

Prepend a new section after the heading block in `CHANGELOG.md` (directly above `## 0.3.0`):

```markdown
## 0.5.0 — 2026-04-15

### Added
- `InstrumentRegistry` module (`xtb_api.instruments`) — persistent JSON cache
  of symbol → instrument-ID mappings, with `populate()` helper that downloads
  the full symbol list and matches against a caller-supplied universe
  (with dot-less fallback: `BRK.B.US → BRKB.US`).
- `XTBAuth` public alias for `xtb_api.auth.AuthManager` — cleaner import path
  for downstream users. Old name continues to work.
- `XTBClient.buy()` / `XTBClient.sell()` now populate `TradeResult.price` by
  polling `get_positions()` after a successful fill (3 attempts, 1 s spacing,
  falls back to `None` if the position does not appear in time).

### Changed
- `XTBClient.buy()` / `XTBClient.sell()` reject `volume < 1` with a clear
  error message (`insufficient_volume: ...`) instead of forwarding an invalid
  value to the gRPC endpoint. Integer rounding rule: `int(volume + 0.5) < 1`.
```

(Note: `0.4.0` section — if missing — is unrelated to this release; do not invent one.)

- [ ] **Step 3: Run version tests**

Run: `pytest tests/test_version.py -v`
Expected: PASS — `__version__` now reads `0.5.0` from installed metadata (after `pip install -e .` picks it up).

If running in an existing virtualenv where the package was installed before the bump, re-run `pip install -e . --no-deps` to refresh metadata, then re-run tests.

- [ ] **Step 4: Commit**

```bash
git add pyproject.toml CHANGELOG.md
git commit -m "chore: bump version to 0.5.0 and draft changelog"
```

---

## Task 2: Add `XTBAuth` alias
...
## Task 3: Volume validation on `buy` and `sell`
...
## Task 4: Populate `TradeResult.price` via fill-price poll
...
## Task 5: Persistent `InstrumentRegistry` module
...
## Task 6: Verify `__init__.py` public surface
...
- [ ] **Step 3: Build the wheel locally**

Run: `python -m build` (or `hatch build`)
Expected: `dist/xtb_api_python-0.5.0-py3-none-any.whl` is produced without errors.

- [ ] **Step 4: Install into the bot's virtualenv for the companion plan**

From `/home/liske/Projects/xtb-investor-pro`:

```bash
.venv/bin/pip install -e /home/liske/Projects/xtb-api-python
.venv/bin/python -c "import xtb_api; print(xtb_api.__version__)"
```
...
gh pr create --title "v0.5.0: broker-adapter support (volume validation, fill-price, instruments)" --body "$(cat <<'EOF'
## Summary
...
Version: 0.4.0 → 0.5.0 (minor, purely additive).
...
- [x] Installed into xtb-investor-pro's venv; `__version__ == "0.5.0"`
...
- [ ] `CHANGELOG.md` has a `## 0.5.0 — 2026-04-15` section.
